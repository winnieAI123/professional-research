...
> **Finance 公司搜索策略（12 章）：**
> 参考 `references/type2_company_research.md` 中的 "Pipeline: Finance Company" 部分获取完整 12 章 query 模板。
> **注意**：Finance 公司必须做牌照验证 — `site:[regulator_site] "[company]"`
...
> **核心判断（Key Takeaways）最后写**：在所有 7 个章节生成完毕后，综合全文提炼核心判断，放在报告最前面。不另外搜索数据。
