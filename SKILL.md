...
> **Rule 7: Agent 用自身能力撰写报告内容，不调用 LLM API 脚本生成文本。**
> Type 1-4（产品 / 公司 / 行业 / 趋势）的报告内容生成方式：
> - Agent 读取该章节的模板片段 + 该章收集的数据 → 直接用 Write 工具撰写该章 Markdown
> - **1 章 1 次写入**，每章独立成文件（`chapter_01.md`、`chapter_02.md`…），最后拼接
> - ⛔ 严禁调用 `scripts/llm_client.py`、`run_report_gen.py`、`gen_all_chapters.py` 等脚本去生成报告文本
> - ⛔ 严禁临时写 Python 脚本调 Gemini/OpenAI/DeepSeek API 来"写报告"
>
> **例外**：Type 5（政策）、Type 6（学术简报）、Type 9（榜单）、Type 10（热点）的**端到端脚本**仍可使用（脚本内部会调 LLM 做精筛/摘要）。这些脚本需要 `GEMINI_API_KEY`。如果未配置，走降级路径——Agent 按 Phase 工作流手动撰写（详见各 Type 章节）。
>
> **为什么这样设计**：Agent 自身的模型能力已经足够撰写高质量叙述性分析。脚本化调 LLM 的历史价值在于"自动化批量生成"，但代价是灵活性差、调试困难、依赖外部 API 稳定性。让 Agent 直接写，可以根据数据质量动态调整每章深度，遇到好数据多展开、遇到数据稀疏坦诚标注。

> **Rule 8: Do NOT create custom generation scripts — 用 Write 工具逐章撰写。**
> 你不能自己写 `generate_xxx_chapters.py` 之类的自定义脚本来生成报告。必须按 Phase 1→2→2.5→3→4 工作流**逐步执行**：
> - Phase 2: 每章逐个搜索 + 收集数据（用 collect_*.py 或 web search 都可以）
> - Phase 3: 每章**单独用一次 Write 工具**写入 `chapter_XX.md`（不要把多章合并到一个 Write 调用）
> - **绝对不要**把多章打包到一个脚本/一次写入中（如把 1-2 章或 3-6 章合并）
>
> 原因：多章合并 = 共享 token 预算 = 每章内容被压缩 = 报告质量下降。1 章 1 次 Write 是硬性要求。
...
4. **Agent 读取模板 + 收集的数据 → 用 Write 工具逐章撰写 Markdown**（不再调 Gemini API 脚本）
...
|----------|-------------|------------|
| `GEMINI_API_KEY` | **可选** — 仅 Type 5/6/9/10 端到端脚本需要；Type 1-4 的报告撰写已改由 Agent 自身能力完成 | Google AI Studio |
| `TAVILY_API_KEY` | All types（搜索 + 全文提取） | tavily.com |
| `TWITTER_API_KEY` | Type 4 (Trend), Type 7 (KOL Digest) | twitterapi.io |

### Python Dependencies

```bash
python -m pip install tavily-python arxiv PyMuPDF feedparser python-docx requests
```

> **注**：`google-genai` 已不再被 Type 1-4 的报告生成流程依赖。如果你使用 Type 5/6/9/10 的端到端脚本（run_paper_briefing.py / run_leaderboard.py / run_policy_domestic.py / collect_hot_take.py），仍需安装 `google-genai`：
> ```bash
> python -m pip install google-genai  # 仅端到端脚本需要
> ```
...
| Key | 影响范围 | 缺失后果 |
|-----|---------|---------|
| `GEMINI_API_KEY` | **可选** — 仅 Type 5/6/9/10 端到端脚本需要 | Type 1-4 不受影响（Agent 自己写报告）；Type 5/6/9/10 改走 Agent 手动流程（按 Phase 工作流，用 collect_*.py 抓数据 + Write 工具撰写） |
...
ℹ️ GEMINI_API_KEY — 可选（仅 Type 5/6/9/10 端到端脚本需要）
   → Type 1-4（产品/公司/行业/趋势）不需要此 key，Agent 自己写报告
   → 如果你要跑 Type 5/6/9/10，申请地址：https://aistudio.google.com/apikey（免费）

❌ TAVILY_API_KEY — 用于高质量搜索和全文提取（推荐配置）
...
| 缺失 Key | 降级方案 | 质量影响 |
|----------|---------|---------|
| `GEMINI_API_KEY` | ✅ **Type 1-4 标准模式** — Agent 用 Write 工具逐章撰写报告（这是当前默认流程，不是降级）；⚠️ **Type 5/6/9/10 降级** — 端到端脚本不可用，改走 Agent 手动流程（collect_*.py 抓数据 + Write 撰写） | Type 1-4 无影响；Type 5/6/9/10 损失脚本化精筛/摘要的效率优势，但内容质量由 Agent 能力保障 |
...
> **NEVER** batch all chapters into one script call or one Write. Hardware research **MUST** use the per-chapter workflow below (same pattern as Type 2 Company Research) — Agent 用 Write 工具逐章撰写，每章独立成文件。
...
> **NEVER** batch all 15 chapters into one Write call or one script call for Software product research.
> 15 章共享 token 预算 = 每章内容被压缩 = 扁平 bullet list 泛滥。Software research **MUST** use the per-chapter Write workflow below.
...
For EACH chapter, **Agent 用 Write 工具单独撰写该章并写入 `chapter_XX.md`**（不要用 LLM 脚本生成）。每次撰写必须遵守以下写作风格指令：

> **软件产品写作风格指令（Software 每章撰写必须遵守）：**
>
...
> **15 次写入，1 章 1 次 Write。第 9 章（最终判断）最后写**，综合前 14 章提炼。
...
> **NEVER** batch all chapters into one Write call or one script call. This causes catastrophic data loss：
> 一次 Write 写完全部章节 = 共享 token 预算 = 大部分搜索数据被压缩丢弃 → report full of "未找到数据"。
...
For EACH chapter, **Agent 用 Write 工具单独撰写该章并写入 `chapter_XX.md`**（不要调 LLM 脚本）。每次撰写必须综合以下要素：
- **写作风格指令（MUST 遵守！）**:
  - **Tech 公司**：参照 `references/type2_company_research.md` 的"写作风格总则"
  - **Finance 公司**：使用下方的"金融公司写作风格指令"
- The specific chapter's template section (including the `<!-- 写作指引 -->` comments)
- ALL data points you collected for that chapter（直接读，不需要打包给 LLM）
- 反编造规则：没数据 = "截至研究日期，该数据尚未公开披露"（但不要因为个别数据缺失就停止分析）
- 关键数据必须自然引用来源（如"据管理层在2025科技日披露"）。**所有 URL 和详细出处只写在报告最末尾的"数据来源"章节，不要在每章末尾写"### 数据来源"小节**

> **金融公司写作风格指令（Finance 公司每章撰写必须遵守）：**
>
...
> **NEVER generate the entire report in one Write call.** Tech = 7 章，Finance = 12 章，每章一次 Write。
...
> **NEVER** batch all chapters into one Write call or one script call.
> 一次 Write 写完全部章节 = 共享 token 预算 = 大部分搜索数据被压缩丢弃 → report filled with generic statements。
...
For EACH chapter, **Agent 用 Write 工具单独撰写该章并写入 `chapter_XX.md`**（不要调 LLM 脚本）。每次撰写综合以下要素：
- 该章的模板 section（包含表格结构和占位符）
- 该章收集到的所有数据（直接读，不需要打包给 LLM）
- 反编造规则：没数据 = "截至研究日期，该信息尚未公开披露"
- 关键数据必须自然引用来源（如"据 Grand View Research 预测"），所有 URL 汇总到末尾数据来源部分

> **NEVER generate the entire report in one Write call.** One Write per chapter ensures each chapter gets full context.
...
> **NEVER** batch all data into one Write call or one script call.
> Trend analysis has data from 4 sources (Twitter + Substack + Web + arXiv). A one-shot generation truncates most of it.
> Trend analysis **MUST** use the workflow below — Agent 用 Write 工具逐章撰写，每章独立成文件。
...
每章**单独用 Write 工具撰写一次**（写入 `chapter_XX.md`），综合该章所需的数据子集：
...
每章撰写规则：
- 读取该章的模板 section + 对应数据子集（Agent 自己读，不需要打包给 LLM）
...
> **NEVER generate the entire 8-chapter report in one Write call.** One Write per chapter.
...
> **⚠️ API Key 降级路径**：本 Type 的端到端脚本 `run_policy_domestic.py` 内部会调 Gemini 做精筛/摘要，需要 `GEMINI_API_KEY`。
> 如果未配置 key 或脚本失败，改走 Agent 手动流程：用 `collect_search.py` 抓数据 + Tavily `site:` 限定搜索官方源 → Agent 用 Write 工具按模板逐章撰写。
...
**🛑 执行方式：只需跑一条命令（需要 GEMINI_API_KEY）**
```bash
...
> **⚠️ API Key 降级路径**：本脚本依赖 `GEMINI_API_KEY`（flash 精筛 + pro 摘要）。
> 如果未配置 key 或脚本失败，改走 Agent 手动流程：
> 1. 用 `collect_rss.py` 或 `collect_arxiv.py` 抓取 RSS/arXiv 论文列表
> 2. Agent 读标题/摘要做关键词匹配和精筛（自己判断价值）
> 3. Agent 用 Write 工具为每篇高价值论文生成 200-400 字中文摘要
> 4. 调 `generate_paper_briefing.py`（纯 Word 生成器，无 LLM）输出 docx
...
> **⚠️ API Key 降级路径**：本 Type 的 `run_leaderboard.py` 内部调 `analyze_leaderboard.py` 做 6 个文字段（宏观格局/核心洞察/赛道分析/技术壁垒/机会筛选/结论）的 Gemini narrative 生成，需要 `GEMINI_API_KEY`。
> 如果未配置 key 或脚本失败：
> 1. 数据采集和图表部分（`collect_leaderboard.py` + `charts_leaderboard.py`）仍可正常运行（不依赖 LLM）
> 2. 6 个文字段改由 Agent 用 Write 工具按 `templates/leaderboard_analysis.md` 模板撰写
> 3. 最后调 `report_leaderboard.py`（纯 Word 生成器）输出 docx
...
- **Agent 自己读全文做结构化提取**（不再 feed 给 Gemini）
...
**⚠️ Type 1-4 报告由 Agent 用 Write 工具逐章生成（不再调脚本）：**

每章独立一次 Write，写入 `chapter_XX.md`，最后拼接为完整报告。这种方式保证：
- 逐章生成（一章一次 Write，避免 token 预算共享导致内容被压缩）
- Agent 严格遵守反编造规则（没数据就标注"截至研究日期未公开披露"，绝不编造）
- 每个数据点必须标注来源
- 缺失数据如实标注，不留 placeholder

⛔ **DO NOT generate reports by calling `run_report_gen.py` / `llm_client.py` / 临时 Python 脚本调 LLM API** —— Type 1-4 全部走 Agent 撰写。

**例外**：Type 5/6/9/10 仍可使用其专用端到端脚本（如 `run_paper_briefing.py`、`run_leaderboard.py` 等），脚本内部会调 LLM。如果未配置 GEMINI_API_KEY，走 Agent 手动流程。

### Step 8: MD → Word Conversion
拼接所有章节后，调用 `save_report()` 或 `md_to_word.py` 转换为 Word：
...
> **⚠️ API Key 降级路径**：本 Type 的 `collect_hot_take.py` 内部调 Gemini 做搜索关键词生成 + 观点提取 + 综合分析，需要 `GEMINI_API_KEY`。
> 如果未配置 key 或脚本失败，改走 Agent 手动流程：
> 1. Agent 自己生成中英文搜索关键词（5 组）
> 2. 用 web search 搜 Twitter/X 热门推文 + 新闻/博客
> 3. Agent 用 Write 工具按 `templates/hot_take.md` 模板撰写事件概要、各方观点、综合观察
> 4. 目标仍为 5 分钟内出结果
...
| 包名 | 用途 | 使用脚本 | 必需性 |
|------|------|---------|---------|
| `python-docx` | Word 报告生成 | `generate_report.py`, `collect_earnings.py` | 必需 |
| `requests` | HTTP 请求 | 多个脚本 | 必需 |
| `beautifulsoup4` | HTML 解析 | `collect_earnings.py`, `collect_financial_deep.py` | 必需 |
| `pdfplumber` | PDF 文本提取 | `collect_financial_deep.py`, `collect_nas.py` | 必需 |
| `PyPDF2` | PDF 文本提取 (备选) | `collect_nas.py` (fallback) | 可选 |
| `akshare` | A股财务数据 | `collect_financials.py` | 必需（A股研究） |
| `yfinance` | 美股/港股数据 | `collect_financials.py`, `collect_earnings.py` | 必需（美股研究） |
| `whoosh` | NAS 全文搜索索引 | `collect_nas.py` | 可选（NAS 搜索） |
| `tavily-python` | Tavily 搜索/提取 API | `collect_search.py` | 必需（搜索） |
| `feedparser` | RSS 解析 | `run_arxiv_pipeline.py` | 必需（Type 6） |
| `google-genai` | Gemini LLM API | `llm_client.py`（Type 5/6/9/10 端到端脚本用） | **可选**（仅 Type 5/6/9/10 需要） |
| `matplotlib` | 图表生成 | 报告图表 | 可选 |
| `pandas` | 数据处理 | 财务数据分析 | 可选 |
